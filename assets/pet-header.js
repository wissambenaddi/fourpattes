(() => {
  ...
  function initMegaMenus(header) {
    ...
    qsa('[data-pet-mega-item]', header).forEach((item) => {
      ...
      if (panel.querySelector('[aria-current="page"]')) item.classList.add('is-current');
    ...
    const closeForViewportChange = () => closeMegaMenus(header);
    if (desktopMedia.addEventListener) desktopMedia.addEventListener('change', closeForViewportChange);
    else desktopMedia.addListener(closeForViewportChange);
  }
